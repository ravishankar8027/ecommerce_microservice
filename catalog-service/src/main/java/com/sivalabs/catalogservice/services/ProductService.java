...
public class ProductService {
    ...
    public List<Product> findAllProducts() {
		 ...
		 * getInventoryLevelsWithFeignClient(); final List<Product> availableProducts =
		 * products.stream() .filter(p -> inventoryLevels.get(p.getCode()) != null &&
		 * inventoryLevels.get(p.getCode()) > 0) .collect(Collectors.toList());
		 */
        return products;
    }
